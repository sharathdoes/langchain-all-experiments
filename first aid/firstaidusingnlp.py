...
for document in documents:
    ...
    # Ensure bag and output_row have consistent shapes
    assert len(bag) == len(words), f"Shape mismatch: bag length {len(bag)} vs words length {len(words)}"
    assert len(output_row) == len(classes), f"Shape mismatch: output_row length {len(output_row)} vs classes length {len(classes)}"

    training.append([bag, output_row])

# Shuffle the training data
random.shuffle(training)
for i, item in enumerate(training[:5]):  # Print first 5 entries
    print(f"Item {i}: {item}, Length: {len(item)}")

# Convert training to numpy array
training = np.array(training, dtype=object)  # Use dtype=object to handle the variable-length lists
...
# Initialize the optimizer with updated arguments
